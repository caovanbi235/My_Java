...
public class TTTvsBot extends JFrame implements ActionListener
{
	boolean win = false;
	String icon;
	...
	public TTTvsBot(String s, int P1, int P2)
	{
		super(s);
		point1 = P1;
		point2 = P2;
	...
	public int checkWin() {
		updateMatrix();
		if (a[0] != 2 && a[0] == a[1] && a[0] == a[2])
			return a[0];
		if (a[3] != 2 && a[3] == a[4] && a[3] == a[5])
			return a[3];
		if (a[6] != 2 && a[6] == a[7] && a[7] == a[8])
			return a[6];
		
		if (a[0] != 2 && a[0] == a[3] && a[0] == a[6])
			return a[0];
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			TTTvsBot k = new TTTvsBot("TicTacToe vs Bot", point1, point2);
			if (Math.random() >= 0.5)
			{
				k.timer.start();
				k.icon = "O";
	...
	public static void main(String[] args) {
		...
		TTTvsBot k = new TTTvsBot("TicTacToe vs Bot", 0, 0);
		if (Math.random() >= 0.5)
		{
			k.timer.start();
			k.icon = "O";
